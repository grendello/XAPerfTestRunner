...
namespace XAPerfTestRunner
{
	class ProcessRunner
	{
		...
		public LogLevel EchoStandardOutputLevel                              { get; set; } = LogLevel.Message;
		public bool EchoStandardError                                        { get; set; }
		public LogLevel EchoStandardErrorLevel                               { get; set; } = LogLevel.Error;
		...
		void AddArgumentsInternal (bool ignoreEmptyArguments, params string[] arguments)
		{
			...
			for (int i = 0; i < arguments.Length; i++) {
				string argument = arguments [i]?.Trim () ?? String.Empty;
		...
		public bool Run ()
		{
			if (EchoStandardOutput) {
				if (StandardOutputEchoWrapper != null) {
					...
					AddStandardOutputSink (new ProcessStandardStreamWrapper { LoggingLevel = EchoStandardOutputLevel});
